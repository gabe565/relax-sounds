	...
	return func(e *core.RequestEvent) error {
		...
		if hasRangeHeader && e.Request.Method == http.MethodGet {
			if chunkStart == 0 {
				...
				if err := entry.LoadPositions(e.Request.Context(), m.valkey); err != nil {
					slog.Error("Failed to load positions", "error", err)
				}
			}
			...
			if m.valkey != nil {
				if err := entry.StorePositions(e.Request.Context(), m.valkey); err != nil {
					slog.Info("Failed to store positions", "error", err)
				}
			}
		}
